...
- 사용 알고리즘
    - DP 기반의 discriminative models의 parameter들을 훈련시킴 (conjugate relationship or approximate method의 대안)
        - why?
        ...
        - 제안 모델 : Maximum Margin Dirichlet Process Mixture Model (MMDPM) (Crammer et al. 2006)
          - 방법
            - Dirichlet Process를 통해 data point들의 assignment parameter 추론
            - 사전지식으로 활용되는 $\{ \theta_k \}^n_{i=1}$은 gibbs sampling을 이용해 추론 
            
$$
p(\theta|\bf{x}_k, \beta) = p(\theta_k|\beta) \prod_{i=1}^{|\bf{x_k}|} p(\bf{x_k{_i}}|\theta_k) \quad (8)
$$
            - 이 때, conjugate relationship을 활용하기에는 이론적, 현실적인 제약이 있을 수 있기 때문에 conditional likelihood model의 일부를 DPM이 아닌 SVM classifier로 바꿈

$$p(\bf{x}_i|\theta_k) \propto exp(\bf{X}^T_i \theta_k - \lambda||\theta_k||^2) \quad (9)$$            

          - Contribution :
                1) Optimize a conditional model
                2) learn model parameters online(?)
                3) yield higher clustering accuracy with fast speed.
          - Gibbs sampling과 Online Maximum Margin leanring을 오가며 parameter를 학습함(EM-algorithm의 E/M step 처럼)
            
          - DPMM(Dirichelt Process Mixture Model)
$$$$
            - Drichelt Process를 통해 $\theta_k$를 generating
          - 제안 모델에서는 DPM 대신 SVM classifier로 대체하여 사용 식(9)  
            
            
---

![DPM graphical model(https://statsbot.co/blog/bayesian-nonparametrics/)](../img/DPM.jpg)
...
### Rstan 설치 - 설치 내용이 빠르게 바뀌므로 1번 링크 참고
